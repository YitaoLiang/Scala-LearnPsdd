...
object VtreeLearner {
  ...
  /**
    ...
    * @return
    */
  def learnMetisTopDown(data: Data, ufactor: Int=1, out: String, entropyOrder: Boolean = false): VtreeNode = {
    val (mi,ent) = MutualInformation.pairwiseCalculations(data,miCalc = true, entCalc = entropyOrder)
    ...
    def splitOrig(vars: Array[Int], context: Array[Int]): (Array[Int],Array[Int]) = {
    ...
    def split(vars: Array[Int], context: Array[Int]): (Array[Int],Array[Int]) = {
      val (g1,g2) = splitOrig(vars, context)
      if (entropyOrder){
        val ent1 = MutualInformation.setEnt(g1,ent,dataVarsIndices)
        val ent2 = MutualInformation.setEnt(g2,ent,dataVarsIndices)
        if (ent1<ent2)
          (g1,g2)
        else
          (g2,g1)
  ...
  /**
    ...
    * @return
    */
  def learnBlossomBottomUp(data: Data, out: String, entropyOrder: Boolean=false): VtreeNode = {
    ...
    def combineOrig(nodes: Array[Array[Int]]): Array[Array[Int]] = blossomCombine(nodes.length, {(i1,i2) => vars2mi(nodes(i1),nodes(i2))}, out)

    def combine(nodes: Array[Array[Int]]): Array[Array[Int]] = {

      val pairs = combineOrig(nodes)
      if (entropyOrder) {
        pairs.map { pair =>
          if (pair.length == 1)
            pair
            ...
            val g1 = pair.head
            val g2 = pair(1)
            val ent1 = MutualInformation.setEnt(nodes(g1), ent, dataVarsIndices)
            val ent2 = MutualInformation.setEnt(nodes(g2), ent, dataVarsIndices)
            if (ent1 < ent2)
              Array(g1, g2)
            else
              Array(g2, g1)
